...
module.exports = {
    ...
    create: async function (doc, data, user) {
        ...
        if (print_type == "QUOTE" || print_type == "CONTRACT") {
            ...
            doc.font('Roboto').opacity(0.7).text(data.rental_data.tax_rate.tax_code || "GST", 403, doc.y - 1, { lineBreak: false }).text((data.rental_data.tax_rate.percentage + '%') || currency_type + "0.00", 480, doc.y, { width: 120, align: 'left', lineBreak: false }).opacity(1).text(tax ? `${currency_type}${parseFloat(`${tax}`.includes(",")? tax?.replaceAll(",",""):tax).toFixed(2)}` : currency_type + "0.00", 0, doc.y - 10, { align: 'right' });
            ...
            (print_type == "QUOTE"||print_type == "CONTRACT") && doc.roundedRect(392, doc.y + 2, 180, 35, 0).fillAndStroke('#f2f2f2', '#f2f2f2');
            doc.fill('black').stroke();
            doc.font('Roboto-bold').fontSize(13).opacity(0.7).text(("TOTAL"), 403, doc.y + 1, { lineBreak: false }).opacity(1).text(total ? `${currency_type}${parseFloat(`${total}`.includes(",")? total?.replaceAll(",",""):total).toFixed(2)}` : currency_type + "0.00", { align: "right" }).stroke();
