...
// "use strict";
// https://jshint.com/
//this code copied from pdfkit-table and modified here
...
class PDFDocumentWithTables extends PDFDocument {
   ...
   * @param {Function} callback 
   */
  table(table, options, callback, isLogistic) {
      ...
      try {
        ...
        const computeRowHeight = (row) => {
          ...
          row.forEach((cell, i) => {
            ...
            let allowedExtensions = /(\.jpg|\.jpeg|\.jfif|\.pjpeg|\.pjp|\.png|\.svg|\.webp)$/i;

            //cell height 
            let cellHeight;
            if (cell?.type === "line") {
              cellHeight = 0;
            }else if(cell?.rowHieght==="summary"){
              cellHeight = this.heightOfString(text, {
                width: columnSizes[i] - (cellp.left + cellp.right),
                align: 'left',
              });
              cellHeight = (cellHeight+28)/3;
            }
            else if (cell === undefined || check || cell === "square_box" || cell === "rect_box") {
              cellHeight = 9.9609375;
            }
            else if (allowedExtensions.exec(text) && i == "0") {
              cellHeight = 27;
            }
            else {
              cellHeight = this.heightOfString(text, {
          ...
          if (table.headers.length > 0) {
            ...
            if (typeof table.headers[0] === 'string') {
              ...
              table.headers.forEach((dataHeader, i) => {
                ...
                let { label, width, renderer, align, headerColor, headerOpacity, headerAlign, padding, color } = dataHeader;
                // check defination
                ...
                // write
                this.fillColor(color).text(label,
                  lastPositionX + (cellPadding.left),
                  startY, {
        ...
        table.datas.forEach((row, i) => {
          ...
          const rectRow = {
            ...
            height: isLogistic? rowHeight + columnSpacing+1:rowHeight + columnSpacing,
          };
          ...
          table.headers.forEach((dataHeader, index) => {
            ...
            // ------------------------------------------------------------------------------
            if (row?.qty?.label || row?.img?.label) {
              this.text(text,
                lastPositionX + (cellPadding.left),
                startY + topTextToAlignVertically, {
                width: 250 + width - (cellPadding.left + cellPadding.right),
                align: 'left',
              });
            } else if (text == "square_box") {
              this.lineJoin('miter')
                .rect(lastPositionX + (cellPadding.left),
                  startY + topTextToAlignVertically, 10, 10).lineWidth(0.1).opacity(0.5)
                .stroke("black");
              this.stroke("#f2f2f2").opacity(1);
            } else if (text == "rect_box") {
              this.lineJoin('miter')
                .rect(lastPositionX + 10.5 + (cellPadding.left),
                  startY + (-2) + topTextToAlignVertically, 35, 12).lineWidth(0.1).opacity(0.5)
                .stroke("black");
              this.stroke("#f2f2f2").opacity(1);
            } else if (row?.logisticDate) {
              this.opacity(0.5);
              this.text(text,
                lastPositionX + (cellPadding.left),
                startY + topTextToAlignVertically, {
              ...
              if (index == "0" && allowedExtensions.exec(text)) {
                ...
                // Render table's all data from here
                if (row && row.customization && row.customization.customClass == "tableRowSubHeader") {
                  if (text == "line") {
                    this.underline(0,
                      startY + topTextToAlignVertically,
                      600, 3, { color: '#bdbdbd' });
                    ...
                    this.text(text,
                      ...
                      startY + topTextToAlignVertically, {
                      width: tableWidth-20,
                      align: (row && row.customization && row.customization.customAlign) ? row.customization.customAlign : align,
                    });
                  }
                } else {
                  this.opacity(1).text(text,
                    lastPositionX + (cellPadding.left),
                    startY + topTextToAlignVertically, {
                    width: width - (cellPadding.left + cellPadding.right),
                    align: align,
